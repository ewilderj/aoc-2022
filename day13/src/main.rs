...
impl Signal {
    ...
    fn compare(l: &Signal, r: &Signal) -> i32 {
        match (l, r) {
            (Signal::I(a), Signal::I(b)) => return (b - a).signum(),
            (Signal::I(_), Signal::L(_)) => return Self::compare(&l.promote(), r),
            (Signal::L(_), Signal::I(_)) => return Self::compare(l, &r.promote()),
            (Signal::L(a), Signal::L(b)) => {
                let mut i = 0;
    ...
    fn promote(&self) -> Signal {
        Signal::L(vec![self.clone()])
    }

    fn marker(n: i32) -> Signal {
        Signal::I(n).promote().promote()
    }
}
...
fn main() {
    ...
    l.push(Signal::marker(2));
    l.push(Signal::marker(6));
    l.sort();

    let (m2, m6) = (Signal::marker(2), Signal::marker(6));
